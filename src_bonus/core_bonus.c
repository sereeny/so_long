...
/*   Updated: 2024/11/09 13:28:25 by ssandova         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (mlx->player)
	{
		...
		if (m == 'w')
			mlx->player = mlx_tex_to_img(mlx->mlx, mlx->player,
					"textures/player_b.png");
		else if (m == 's')
			mlx->player = mlx_tex_to_img(mlx->mlx, mlx->player,
					"textures/player_f.png");
		else if (m == 'a')
			mlx->player = mlx_tex_to_img(mlx->mlx, mlx->player,
					"textures/player_l.png");
		else if (m == 'd')
			mlx->player = mlx_tex_to_img(mlx->mlx, mlx->player,
					"textures/player_r.png");
	}
	if (!mlx->player)
		close_mlx(mlx, 2);
...
static void	rem_and_new(t_mlxinfo *mlx, int x, int y)
{
	char	*display;
	char	*amount;
	...
	mlx->moves += 1;
	amount = ft_itoa(mlx->moves);
	display = ft_strjoin("Moves: ", amount);
	if (mlx->text)
		mlx_delete_image(mlx->mlx, mlx->text);
	mlx->text = mlx_put_string(mlx->mlx, display, 0, 0);
	free(amount);
	free(display);
}
...
void	my_keyhook_b(mlx_key_data_t key, void *mlx)
{
	...
	if ((dummy-> map_info->map_cont[y][x] == 'E' && dummy->map_info
		->collectibles == 0))
		close_mlx(mlx, 1);
	else if (dummy->map_info->map_cont[y][x] == 'X')
		close_mlx(mlx, 3);
